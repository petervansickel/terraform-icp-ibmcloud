...
Download the IBM Cloud Private binaries and save them to the `icp-install` directory.  
...
1. Copy the binary package (specified in `image_location`) to `/tmp` on the boot node.
...
| `os_reference_code`   | yes           | OS to install on the VSIs.  Use the [API](https://api.softlayer.com/rest/v3/SoftLayer_Virtual_Guest_Block_Device_Template_Group/getVhdImportSoftwareDescriptions.json?objectMask=referenceCode) to determine valid values. |
